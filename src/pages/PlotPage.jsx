                ...
                return (
                  <div
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        navigate(
                          `/plots/${encodeURIComponent(villageId)}/${encodeURIComponent(
                            p.plotId
                          )}`
                        );
                      }
                    ...
                    className={`bg-white rounded-xl shadow hover:shadow-lg p-4 border transition transform ${
                      p.deleted ? "ring-0" : ""
                    } cursor-pointer`}
                  >
            ...
            {!loading && deletedPlots && deletedPlots.length > 0 && (
              <div className="sm:col-span-2 mt-3 text-center">
                ...
                >
                  {showDeleted
                    ? "Hide deleted plots"
                    : `View deleted plots (${deletedPlots.length})`}
                </button>
              </div>
            )}
          </div>
