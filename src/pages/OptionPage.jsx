...
/**
 * OptionPage.jsx — updated/cleaned
 * - Replaced mojibake chars (… ← ✓ ▲ ▼ etc.)
 * - Replaced hard-coded host URLs with `${API_BASE}` where applicable
 * - Small clarity/refactor changes; preserved original behavior
 */
...
export default function OptionPage() {
    ...
    (async function load() {
      ...
      try {
        ...
        if (res.status === 404) {
          if (!mounted) return;
          setStages([]);
          // treat 404 as "no stages" (not a fatal error)
          setError(null);
          return;
        }
        if (!res.ok) {
  ...
  const getSubId = (ss) =>
    ss.stageId ?? ss.subStageId ?? ss.sub_stage_id ?? ss.id ?? ss.sub_id ?? ss.subId ?? ss.name;
  ...
  async function toggleShowDeletedInline(stageId) {
    ...
    // Use backend route for deleted sub-stages
    try {
  ...
  // ---------- create / update / delete (including position) ----------
  async function handleCreateStage(e) {
    ...
    try {
      const payload = {
        ...
        deleted: false,
        stages: createSubstages
          .map((s) => ({ name: (s.name || "").trim(), desc: (s.desc || "").trim() || undefined }))
          .filter((x) => x.name),
      };
      const res = await fetch(`${API_BASE}/options/insert`, {
        method: "POST",
        headers: authHeaders(),
      ...
      if (inserted) {
        ...
        const insertedId =
          inserted.optionId ?? inserted.option_id ?? inserted.stageId ?? inserted.stage_id ?? inserted.id;
        if (insertedId != null) {
          setExpandedStageIds((prev) => {
            const copy = new Set(prev);
          ...
          if (found) {
            ...
            setTimeout(() => {
              try {
                const el = stageRefs.current[fid];
      ...
      if (fresh) {
        ...
        if (parent) {
          ...
          if (!subId && payload && payload.name) {
            ...
            const candidates = (parent.stages ?? []).filter(
              (ss) => String(ss.name ?? "").trim() === String(payload.name ?? "").trim()
            );
          ...
          setTimeout(() => {
            try {
              ...
              if (pel && typeof pel.scrollIntoView === "function")
  ...
  async function submitEditSubstage(e) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/ostages/${encodeURIComponent(stageId)}/${encodeURIComponent(subStageId)}`,
        {
          method: "PUT",
          headers: authHeaders(),
  ...
  async function performDeleteSubstage(stageId, subStageId) {
    try {
      ...
      setSelectedSubstages((prev) => {
        const c = { ...prev };
        if (c[stageId]) {
          c[stageId].delete(subStageId);
          if (c[stageId].size === 0) delete c[stageId];
        }
        return c;
      });
    } catch (err) {
  ...
  // ---------- deleted stages global toggle (handle 404 as 'no deleted') ----------
  async function toggleDeletedStagesGlobal() {
    ...
    try {
      const res = await fetch(`${API_BASE}/deleted_options`);
      if (res.status === 404) {
  ...
  function renderSubstageRow(stageId, ss, indexInStage) {
    ...
    const isDraggingOver =
      String(pendingSubstageReorder?.stageId) === String(stageId) &&
      pendingSubstageReorder?.insertAt === indexInStage &&
      pendingSubstageReorder?.movedId === subId
        ? true
        : false;
      ...
      >
        <div className="flex items-center justify-between gap-3 min-w-0">
          <div className="flex items-center gap-3 min-w-0">
            {perStageSelecting &&
              (ss.deleted ? <input type="checkbox" disabled /> : <input type="checkbox" checked={subSelected} onChange={() => toggleSubstageCheckbox(stageId, subId)} />)}
            <div className="min-w-0">
              <div className={`${ss.deleted ? "line-through text-gray-400" : "text-gray-800"} font-medium truncate`}>
                {ss.name ?? `Sub ${subId}`}
              </div>
          ...
          <div className="flex items-center gap-2">
            {!perStageSelecting && (
              <>
                <button
                  onClick={() => setEditSubstage({ stageId, subStageId: subId, name: ss.name ?? "", desc: ss.desc ?? "" })}
                  className="px-2 py-1 rounded bg-indigo-50 text-sm"
                >
                  Edit
                </button>
              </>
  ...
  async function handleSubDrop(e, stageId, targetIndex) {
    ...
    if (!subDragInfo || String(subDragInfo.stageId) !== String(stageId)) {
      setSubDragInfo(null);
      setSubDragOver({ stageId: null, index: null });
      return;
    }
  ...
  function findSubIdByNameDesc(stageObj, name, desc) {
    ...
    let found = stageObj.stages.find(
      (ss) => String((ss.name ?? "")).trim() === trimmedName && (trimmedDesc == null || String((ss.desc ?? "")).trim() === trimmedDesc)
    );
    if (!found) {
  ...
  async function confirmSubstageReorder() {
    ...
    try {
      ...
      if (!movedId) {
        throw new Error(
          "Reorder failed: could not determine sub-stage id for the moved item. Please refresh or edit the sub-stage to ensure it has an id."
        );
      }
    ...
    try {
      if (deleteConfirm.type === "stage") {
        ...
        for (const id of deleteConfirm.ids || []) {
          try {
            const res = await fetch(`${API_BASE}/options/${encodeURIComponent(id)}`, {
              method: "DELETE",
              headers: authHeaders(),
            });
            if (!res.ok) {
        ...
        for (const sid of deleteConfirm.ids || []) {
          try {
            const res = await fetch(
              `${API_BASE}/ostages/${encodeURIComponent(deleteConfirm.stageId)}/${encodeURIComponent(sid)}`,
              { method: "DELETE", headers: authHeaders() }
            );
            if (!res.ok) {
              const txt = await res.text().catch(() => "");
        ...
        setSelectedSubstages((prev) => {
          const c = { ...prev };
          delete c[deleteConfirm.stageId];
          return c;
        });
  ...
  async function performDeleteStage(stageId) {
    try {
      const res = await fetch(`${API_BASE}/options/${encodeURIComponent(stageId)}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-[#f8f0dc] font-sans">
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
          <div>
            <button onClick={() => navigate("/dashboard")} className="px-3 py-2 border rounded-md bg-white text-sm">
              ← Back
            </button>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            {!globalSelectMode && (
              <button onClick={() => setShowCreatePanel((s) => !s)} className="px-4 py-2 bg-blue-600 text-white rounded-md w-full sm:w-auto text-sm">
                + Add
              </button>
            )}
              ...
              <div className="flex gap-2 w-full sm:w-auto">
                <button
                  ...
                  className={`px-4 py-2 rounded-md ${
                    selectedStageIds.size === 0 ? "bg-red-100 text-red-300 cursor-not-allowed" : "bg-red-600 text-white"
        ...
        {showCreatePanel && (
          <form onSubmit={handleCreateStage} className="bg-white rounded-lg shadow p-4 mb-6">
            ...
            <div className="center">
              <button
                type="button"
                onClick={() => setCreateSubstages((prev) => [...prev, { name: "", desc: "" }])}
            ...
            <div className="mt-3 space-y-2">
              {createSubstages.map((x, i) => (
                <div key={i} className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-6 gap-2 items-center">
                  <input
                    value={x.name}
                    onChange={(e) => setCreateSubstages((prev) => prev.map((v, ii) => (ii === i ? { ...v, name: e.target.value } : v)))}
                    placeholder="Sub-stage name"
                    className="md:col-span-2 p-2 border rounded w-full"
                  />
                  <input
                    value={x.desc}
                    onChange={(e) => setCreateSubstages((prev) => prev.map((v, ii) => (ii === i ? { ...v, desc: e.target.value } : v)))}
                    placeholder="Description"
                    className="md:col-span-3 p-2 border rounded w-full"
                  />
                  <div className="md:col-span-1">
                    <button type="button" onClick={() => setCreateSubstages((prev) => prev.filter((_, ii) => ii !== i))} className="px-2 py-2 border rounded w-full sm:w-auto">
                      Remove
                    </button>
                  </div>
                </div>
            ...
            <div className="mt-4 flex gap-2 flex-col sm:flex-row justify-center">
              <button type="submit" className="px-10 py-2 bg-blue-600 text-white rounded w-full sm:w-auto">
                {creating ? "Creating…" : "Create"}
              </button>
              <button type="button" onClick={() => setShowCreatePanel(false)} className="px-10 py-2 border rounded w-full sm:w-auto">
                Cancel
              </button>
            </div>
          </form>
                  ...
                  >
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        {globalSelectMode && (
                          <input type="checkbox" checked={stageSelected} onChange={() => toggleStageCheckbox(stageId)} className="w-4 h-4" aria-label={stageSelected ? "Deselect stage" : "Select stage"} />
                        )}
                      ...
                      <div className="flex items-center gap-2 mt-3 sm:mt-0">
                        {!globalSelectMode && !expanded && (
                          <>
                            <button onClick={() => setEditStage({ stageId, name: s.name ?? "", desc: s.desc ?? "", deleted: !!s.deleted })} className="px-3 py-1 rounded bg-indigo-50 text-sm">
                              Edit
                            </button>
                            <button onClick={() => toggleExpandStage(stageId)} className="px-3 py-1 rounded bg-gray-50 text-sm">
                              ▼
                            </button>
                          </>
                        )}
                    ...
                    {expanded && (
                      <div className="mt-4 border-t pt-4 space-y-4">
                              ...
                              <>
                                ...
                                {stageSubSelectMode[stageId] && selectedSubstages[stageId] && selectedSubstages[stageId].size > 0 && (
                                  <>
                                    ...
                                    <button onClick={() => deselectAllSubstagesFor(stageId)} className="px-3 py-1 rounded bg-gray-200 text-gray-700 text-sm" title="Deselect all selected sub-stages in this stage">
                                      Deselect all
                                    </button>
                                  </>
                                )}
                        ...
                        <div className="space-y-2 max-h-[50vh] sm:max-h-[40vh] overflow-auto">
                          {activeSubs.length === 0 ? <div className="text-xs text-gray-500">No sub-stages</div> : activeSubs.map((ss, i) => renderSubstageRow(stageId, ss, i))}
                        </div>

                        <div className="mt-2">
                          {!showAddFormFor || showAddFormFor !== stageId ? (
                            <button onClick={() => setShowAddFormFor(stageId)} className="px-3 py-1 rounded bg-green-50 text-sm w-full sm:w-auto">
                              + Add Sub-stage
                            </button>
                          ) : (
                            <form
                            ...
                            >
                              ...
                              <div className="mt-2 flex flex-col sm:flex-row gap-2">
                                <button type="submit" className="px-3 py-1 bg-blue-600 text-white rounded w-full sm:w-auto">
                                  Add
                                </button>
                                <button type="button" onClick={() => setShowAddFormFor(null)} className="px-3 py-1 border rounded w-full sm:w-auto">
                                  Cancel
                                </button>
                              </div>
                            </form>
                        ...
                        {showDeletedLink && (
                          <div className="mt-2 flex flex-col sm:flex-row sm:justify-end items-center gap-3">
                            ...
                            {!showDeletedInExpanded[stageId] && <div className="text-xs text-gray-400" />}
                          </div>
                        )}
                    ...
                    return (
                      <div key={String(id)} className="p-4 border rounded-xl bg-gray-100 ">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button onClick={() => toggleExpandDeletedStage(id)} className="px-2 py-1 rounded bg-gray-100 text-sm" title={isExpanded ? "Collapse" : "Expand"}>
                              {isExpanded ? "▲" : "▼"}
                            </button>
                          </div>
                        </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-4 w-full max-w-md sm:max-w-lg md:max-w-xl mx-3 sm:mx-0">
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    const res = await fetch(`${API_BASE}/options/${encodeURIComponent(editStage.stageId)}`, {
                      method: "PUT",
                      headers: authHeaders(),
                      body: JSON.stringify(payload),
                    });
              ...
              >
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button type="button" onClick={() => setEditStage(null)} className="px-4 py-2 border rounded">
                    Cancel
                  </button>
                </div>
              </form>
            ...
            <div className="bg-white rounded-lg shadow-lg p-4 w-full max-w-md sm:max-w-lg md:max-w-xl mx-3 sm:mx-0">
              ...
              <form onSubmit={submitEditSubstage} className="space-y-3">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button type="button" onClick={() => setEditSubstage(null)} className="px-4 py-2 border rounded">
                    Cancel
                  </button>
                </div>
              </form>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-3 sm:mx-0">
              ...
              <div className="text-sm text-gray-700 mb-4">
                Move
                " <span className="font-medium">{pendingSubstageReorder.moved?.name ?? pendingSubstageReorder.movedId}</span> " to position{" "}
                <span className="font-medium">{pendingSubstageReorder.insertAt}</span> in stage{" "}
                <span className="font-medium">{stages[pendingSubstageReorder.stageIndex]?.name ?? pendingSubstageReorder.stageId}</span>?
              </div>
              {error && <div className="text-sm text-red-600 mb-3">{error}</div>}
              <div className="flex justify-end gap-2">
                <button onClick={cancelSubstageReorder} disabled={!!persistingSubstage} className="px-4 py-2 border rounded">
                  Cancel
                </button>
                <button onClick={confirmSubstageReorder} disabled={!!persistingSubstage} className="px-4 py-2 bg-blue-600 text-white rounded">
                  {persistingSubstage ? "Saving…" : "Confirm"}
                </button>
              </div>
            </div>
          </div>
        ...
        {deleteConfirm && (
          <div className="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40 p-4">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-3 sm:mx-0">
              ...
              <div className="text-sm text-gray-700 mb-4">
                {deleteConfirm.type === "stage" && <>Are you sure you want to delete stage "<span className="font-medium">{deleteConfirm.name ?? deleteConfirm.stageId}</span>"?</>}
                {deleteConfirm.type === "substage" && <>Are you sure you want to delete sub-stage "<span className="font-medium">{deleteConfirm.name ?? deleteConfirm.subStageId}</span>"?</>}
                {deleteConfirm.type === "multipleStages" && <>Are you sure you want to delete <span className="font-medium">{(deleteConfirm.ids || []).length}</span> selected stages?</>}
                {deleteConfirm.type === "multipleSubstages" && <>Are you sure you want to delete <span className="font-medium">{(deleteConfirm.ids || []).length}</span> selected sub-stages?</>}
              </div>

              {error && <div className="text-sm text-red-600 mb-3">{error}</div>}

              <div className="flex justify-end gap-2">
                <button onClick={() => setDeleteConfirm(null)} disabled={performingDelete} className="px-4 py-2 border rounded">
                  Cancel
                </button>
                <button onClick={performDeleteConfirmed} disabled={performingDelete} className="px-4 py-2 bg-red-600 text-white rounded">
                  {performingDelete ? "Deleting…" : "Delete"}
                </button>
              </div>
            </div>
          </div>
