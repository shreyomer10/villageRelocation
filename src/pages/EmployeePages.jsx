  ...
  const handleConfirmBulkAdd = async () => {
    ...
    try {
      ...
      if (unknownsAcc.length) {
        unknownsAcc.forEach((u) => preFriendly.push(`❗ ${u.name} — unknown villages: ${u.unknowns.join(", ")}`));
      }
      ...
      if (Array.isArray(summary.skipped_existing) && summary.skipped_existing.length) {
        summary.skipped_existing.forEach((s) => {
          ...
          friendly.push(reason ? `⏭️ Skipped: ${id} — ${reason}` : `⏭️ Skipped: ${id}`);
        });
      }
                    ...
                    <ul className="mt-2 space-y-1">
                      {bulkProgress.friendly.map((line, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-sm ${line.startsWith("✅") ? "bg-green-100 text-green-800" : line.startsWith("⚠️") ? "bg-yellow-100 text-yellow-800" : "bg-gray-100 text-gray-800"}`}>
                            {line.startsWith("✅") ? "✓" : line.startsWith("⚠️") ? "!" : "•"}
                          </span>
                          <div className="text-sm">{line.replace(/^✅\s|^⚠️\s|^⏭️\s/, "")}</div>
                        </li>
                      ))}
                    </ul>
                  </div>
