...
export default function RelocationTracker({ stages = [] }) {
  ...
  useEffect(() => {
    ...
    if (selectedStage) window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [selectedStage]);

  // helper to open selected stage either by object or by id/sequence_no
  function openStage(stageOrId) {
    if (!stageOrId) return;
    const stageObj = typeof stageOrId === "object"
      ? stageOrId
      : stages.find((s) => String(s.sequence_no ?? s.id ?? s.stage_id) === String(stageOrId));
    if (stageObj) setSelectedStage(stageObj);
  }
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-lg">
      ...
      <StageProgress
        ...
        onStageChange={(stageNo, subNo) => {
          // allow StageProgress to update the current stage if it emits change events
          if (typeof stageNo === "number") setCurrentStage(stageNo);
          if (typeof subNo === "number") setCurrentSubStage(subNo);
        }}
      />

      {/* small progress bar (visual feedback) */}
      <div className="mt-4 w-full px-4">
        ...
        <div className="text-xs text-gray-500 text-center mt-1">Progress: {Math.round(progressPercent)}%</div>
      </div>
      ...
      {selectedStage && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="relocation-stage-title"
        ...
        >
          <div className="bg-white rounded-xl shadow-lg p-6 w-11/12 sm:w-96 relative">
            ...
            <h2 id="relocation-stage-title" className="text-lg font-semibold mb-4">{selectedStage.name}</h2>
